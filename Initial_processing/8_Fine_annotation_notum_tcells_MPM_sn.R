...
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.4 %in% c(0), 'CD8+ T cells TCF7+',
                             'NA')
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.4 %in% c(3, 8), 'CD8+ T cells TOX+',
                             seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.4 %in% c(6), 'Tfh-like cells',
                             seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.4 %in% c(1), 'CD4+ T cells',
                             seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.4 %in% c(2), 'Tregs',
                             seu$cell_type_fine)
...
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.4 %in% c(7), 'NK cells',
                             seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.4 %in% c(5), 'Contamination',
                             seu$cell_type_fine)
