...
FeaturePlot(seu, features = c('CD3D', 'CD3E', 'TOX', 'FTL'), min.cutoff = 'q05',
...
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.5 %in% c(0, 1, 2, 3, 5, 6),
                             'Monocyte-derived macrophages', 'NA')
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.5 %in% c(4), 'Monocytes',
                             seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.5 %in% c(10), 'cDC1',
                             seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.5 %in% c(13), 'cDC2',
                             seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.5 %in% c(11), 'DC3',
                             seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.5 %in% c(8), 'Cycling cells',
                             seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.5 %in% c(7, 9), 'Contamination',
                             seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.5 %in% c(12), 'Mast cells',
                             seu$cell_type_fine)


#### Subset to DCs ####
dc <- subset(seu, integrated_snn_res.0.5 %in% c(4, 10, 11, 13))
dc <- ScaleData(dc)
...
mdm$original_clusters <- mdm$integrated_snn_res.0.5
mdm <- ScaleData(mdm)
...
# Fine cell type assignment after manual annotation based on DGE
mdm$cell_type_fine <- ifelse(mdm$integrated_snn_res.0.3 %in% c(0), 'MDM', 'NA')
mdm$cell_type_fine <- ifelse(mdm$integrated_snn_res.0.3 %in% c(1), 'MDM M2-like',
                             mdm$cell_type_fine)
mdm$cell_type_fine <- ifelse(mdm$integrated_snn_res.0.3 %in% c(4), 'MDM M1-like',
                             mdm$cell_type_fine)
mdm$cell_type_fine <- ifelse(mdm$integrated_snn_res.0.3 %in% c(3), 'Monocytes',
                             mdm$cell_type_fine)
mdm$cell_type_fine <- ifelse(mdm$integrated_snn_res.0.3 %in% c(2), 'MDM FTL+',
                             mdm$cell_type_fine)
...
#### Add annotation from MDM and DC ####
