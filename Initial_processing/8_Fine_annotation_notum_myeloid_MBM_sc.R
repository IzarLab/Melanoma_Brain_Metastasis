...
# Remove cluster 4 (T-cell doublets)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.3 == 4 | is.na(seu$frequency) == F,
                             'T-cell doublets', NA)
ct <- seu@meta.data %>%
  select('barcode_pat', 'cell_cycle', 'cell_type_fine')
ct <- subset(ct, is.na(ct$cell_type_fine) == F)
write.csv(ct, paste0(path.ct, 'data_', filename, '_celltype_reint_pt1.csv'),
          row.names = F)

seu <- subset(seu, integrated_snn_res.0.3 != 4 | is.na(seu$frequency) == F)
seu <- subset(seu, tcr == 'no_TCR')
seu <- ScaleData(object = seu)
seu <- RunPCA(object = seu, npcs = 75)
ElbowPlot(seu, ndims = 75)
seu <- FindNeighbors(seu, dims = 1:30)
...
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.3 %in% c(0, 1, 3, 4, 8, 9),
                             'Monocyte-derived macrophages', 'NA')
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.3 %in% c(2), 'Microglia',
                             seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.3 %in% c(7), 'cDC1',
                             seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.3 %in% c(6), 'cDC2',
                             seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.3 %in% c(10), 'DC3',
                             seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.3 %in% c(5), 'Cycling cells',
                             seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.3 %in% c(1, 3) & seu$Monocytes1 >
                               0.15, 'Monocytes', seu$cell_type_fine)

#### Subset to DCs to confirm assignment above ####
dc <- subset(seu, integrated_snn_res.0.3 %in% c(6, 7, 10))
dc <- ScaleData(dc)
...
dc$cell_type_fine <- ifelse(dc$integrated_snn_res.0.4 %in% c(1), 'cDC1', dc$cell_type_fine)
dc$cell_type_fine <- ifelse(dc$integrated_snn_res.0.4 %in% c(3), 'DC3', dc$cell_type_fine)
dc$cell_type_fine <- ifelse(dc$integrated_snn_res.0.4 %in% c(2), 'DCs (mixed)', dc$cell_type_fine)

# Save object and labels
saveRDS(dc,paste0(path.ct,'data_',filename,'_DC.rds'))
celltype <- dc@meta.data %>% select('barcode_pat', 'cell_cycle', 'cell_type_fine')
write.csv(celltype, paste0(path.ct,'data_',filename,'_DC_celltype.csv'))


#### Subset to MDMs to identify subpopulations ####
mdm <- subset(seu, cell_type_fine == 'Monocyte-derived macrophages')
mdm$original_clusters <- mdm$integrated_snn_res.0.3
mdm <- ScaleData(mdm)
mdm <- RunPCA(mdm, npcs = 75)
ElbowPlot(mdm, ndims = 75)
mdm <- FindNeighbors(mdm, dims = 1:30)
mdm <- FindClusters(mdm, resolution = 0.15)
mdm <- RunUMAP(mdm, dims = 1:30)
...
# Differential gene expression (DGE) based on clusters
Idents(mdm) <- mdm$integrated_snn_res.0.15
...
# Fine cell type assignment after manual annotation based on DGE
mdm$cell_type_fine <- ifelse(mdm$integrated_snn_res.0.2 %in% c(0), 'MDM', 'NA')
mdm$cell_type_fine <- ifelse(mdm$integrated_snn_res.0.2 %in% c(1, 3), 'MDM M2-like',
                             mdm$cell_type_fine)
mdm$cell_type_fine <- ifelse(mdm$integrated_snn_res.0.2 %in% c(5, 7), 'MDM M1-like',
                             mdm$cell_type_fine)
mdm$cell_type_fine <- ifelse(mdm$integrated_snn_res.0.2 %in% c(4),
                             'Proinflammatory MDM (glycolysis)', mdm$cell_type_fine)
mdm$cell_type_fine <- ifelse(mdm$integrated_snn_res.0.2 %in% c(6),
                             'Proinflammatory MDM (zinc)', mdm$cell_type_fine)
mdm$cell_type_fine <- ifelse(mdm$integrated_snn_res.0.2 %in% c(2), 'MDM FTL+',
                             mdm$cell_type_fine)
...
#### Add annotation from DCs and MDM ####
...
ct<-rbind.data.frame(ct_mdm,ct_dc)
ct$X<-NULL
