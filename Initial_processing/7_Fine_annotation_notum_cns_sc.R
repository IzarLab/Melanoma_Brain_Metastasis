...
#### Fine cell type annotation of CNS cell subset in sc 
...
write.csv(markers, 'data/cell_type_assignment/sc/cns/markers_MBPM_sc_cns.csv', row.names = F)

# Fine cell type assignment after manual annotation based on DGE
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.4 %in% c(2), 'Oligodendrocytes', 'NA')
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.4 %in% c(3), 'Cycling cells', seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.4 %in% c(0), 'Low-quality tumor', seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.4 %in% c(1), 'Neurons', seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.4 %in% c(4), 'CAFs', seu$cell_type_fine)


# Save object and labels
saveRDS(seu, 'data/cell_type_assignment/sc/cns/data_MBPM_sc_cns.rds')
celltype <- seu@meta.data %>%
  select('barcode_all', 'cell_cycle', 'cell_type_fine')
write.csv(celltype, 'data/cell_type_assignment/sc/cns/data_MBPM_sc_cns_celltype.csv')

# DGE based on cell type
Idents(seu) <- seu$cell_type_fine
markers <- FindAllMarkers(seu, only.pos = TRUE, assay = 'RNA', min.pct = 0.25, logfc.threshold = 0.25)
ifelse(!dir.exists(file.path('data/cell_type_DEG/sc/cns')), dir.create(file.path('data/cell_type_DEG/sc/cns'), recursive = T), FALSE)
write.csv(markers, 'data/cell_type_DEG/sc/cns/markers_MBPM_sc_cns_type.csv', row.names = F)
...
pdf(file = 'data/cell_type_DEG/sc/cns/plots_MBPM_sc_cns_heatmap_type.pdf', width = 10, height = 17)
