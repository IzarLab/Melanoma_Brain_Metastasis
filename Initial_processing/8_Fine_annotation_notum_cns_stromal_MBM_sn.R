...
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.4 %in% c(6), 'Oligodendrocytes',
                             'NA')
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.4 %in% c(0, 1, 3, 4, 10),
                             'Low-quality tumor/immune', seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.4 %in% c(2, 9), 'Neurons',
                             seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.4 %in% c(8), 'Astrocytes',
                             seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.4 %in% c(5), 'CAFs',
                             seu$cell_type_fine)
seu$cell_type_fine <- ifelse(seu$integrated_snn_res.0.4 %in% c(7), 'Pericytes',
                             seu$cell_type_fine)
