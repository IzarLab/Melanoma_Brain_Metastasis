...
# Read-in integrated objectand subset
...
markers <- FindMarkers(seu, ident.1 = 'Single cells', ident.2 = 'Single nuclei', assay = 'downsampled', 
                       logfc.threshold = 0, min.pct = 0, max.cells.per.ident = min(table(seu$sequencing)))
markers$gene <- rownames(markers)

# Label different log2FC categories
markers$diffexpressed <- ifelse(markers$p_val_adj < 0.05 & markers$avg_log2FC > 2, 'Upregulated', 
                                ifelse(markers$p_val_adj < 0.05 & markers$avg_log2FC < (-2), 'Downregulated', 'NS'))

# Add top 20 DEGs as labels and save file
tops <- markers %>% filter(p_val_adj < 0.05) %>% top_n(n = 20, wt = avg_log2FC)
tops <- rbind.data.frame(tops, markers %>% filter(p_val_adj < 0.05) %>% top_n(n = -20, wt = avg_log2FC))
markers$label <- ifelse(markers$gene %in% tops$gene, markers$gene, NA)
ifelse(!dir.exists(file.path('data/DEG/sequencing/downsampled_noRPMT/cd8_tox/')), dir.create(file.path('data/DEG/sequencing/downsampled_noRPMT/cd8_tox/'), recursive = T), FALSE)
...
ggplot(avlogexp, aes(x = sc_avglogexp, y = sn_avglogexp)) + 
  ...
  labs(title = paste0('TOX+ CD8+ T cells (R^2: ', round(cor.test(avlogexp$sc_avglogexp, avlogexp$sn_avglogexp, method = 'pearson')$estimate, 2), ')')) + 
